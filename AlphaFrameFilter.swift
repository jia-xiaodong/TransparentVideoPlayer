...
final class AlphaFrameFilter: CIFilter {
    ...
    private static var metalKernel: CIKernel? = {
        do {
			return try CIKernel.kernelByName("alphaFrame")
		} catch {
    ...
    private func render(using renderingMode: RenderingMode, inputImage: CIImage, maskImage: CIImage) -> CIImage? {
        ...
        case .builtInFilter:
            ...
            let backgroundImage = CIImage(color: CIColor(string: "0.0 0.0 0.0 0.0")).imageByCroppingToRect(outputExtent)
        ...
        case .colorKernel:
			...
			return colorKernel.applyWithExtent(outputExtent, arguments: arguments)

        case .metalKernel:
            guard let metalKernel = AlphaFrameFilter.metalKernel else {
                outputError = AlphaFrameFilter.metalKernelError ?? Error.invalidKernel
                return nil
            }
            ...
            let arguments = [inputImage, maskImage]
			return metalKernel.applyWithExtent(outputExtent, roiCallback: roiCallback, arguments: arguments)
        }
    }
}
